#!/usr/bin/env python3
"""
LLMベースの書誌レコードマッチングシステム (Jaro-Winkler最適化版)

このスクリプトはJaro-Winkler距離とLLM（大規模言語モデル）を組み合わせて書誌レコードの
類似性を判定し、クラスタリングを行います。一定以上の類似度を持つレコードは直接マッチング、
一定以下の類似度を持つレコードは直接不一致とし、その間のグレーゾーンのみLLM判定を使用することで
処理効率を向上させています。

特徴:
- Jaro-Winkler距離による高速な初期フィルタリング
- LLMの高度な文脈理解を活用した類似性判定（グレーゾーンのみ）
- レコードペアごとの詳細な類似度分析
...
class Config:
    ...
    def __init__(self):
        # デフォルト設定
        self.similarity_threshold = 0.7  # LLMの類似度判定のしきい値（0-1）
        self.jw_upper_threshold = 0.9    # この値以上なら直接マッチとする
        self.jw_lower_threshold = 0.3    # この値以下なら直接不一致とする
        self.max_retries = 3             # APIコールの最大リトライ回数
        self.retry_delay = 2             # リトライ間の待機時間（秒）
        self.cache_dir = ".cache"        # キャッシュディレクトリ
        self.debug = False               # デバッグモード（詳細な出力）
        self.api_key = None              # OpenAI APIキー

    def load_from_args(self, args):
        """コマンドライン引数から設定を読み込む"""
        if hasattr(args, 'threshold') and args.threshold is not None:
            self.similarity_threshold = args.threshold
        if hasattr(args, 'jw_upper') and args.jw_upper is not None:
            self.jw_upper_threshold = args.jw_upper
        if hasattr(args, 'jw_lower') and args.jw_lower is not None:
...
def extract_key_fields(record: Dict, field_types: Dict[str, str]) -> Dict[str, str]:
    """レコードから重要なフィールドを抽出する"""
    result = {}
    ...
    if "data" in record and isinstance(record["data"], dict):
        ...
        # タイトルフィールドを探す
        for field, field_type in field_types.items():
            if field_type == "TEXT" and "title" in field.lower() and field in data:
                result["title"] = data[field]
                break
        
        # 著者フィールドを探す
        for field, field_type in field_types.items():
            if field_type == "COMPLEMENT_JA" and "author" in field.lower() and field in data:
                result["author"] = data[field]
                break
        
        # 出版社フィールドを探す
        for field, field_type in field_types.items():
            if field_type == "TEXT" and "publisher" in field.lower() and field in data:
                result["publisher"] = data[field]
                break
        
        # 出版日フィールドを探す
        for field, field_type in field_types.items():
            if field_type == "DATE" and ("date" in field.lower() or "pubdate" in field.lower()) and field in data:
                result["date"] = data[field]
                break
    
    return result

def calculate_jaro_winkler_similarity(record1: Dict, record2: Dict, field_types: Dict[str, str]) -> float:
    """
    Jaro-Winkler距離を使用して2つのレコードの類似度を計算する
    タイトルと著者の類似度を重視し、出版社と出版日も考慮する
    """
    # 重要フィールドの抽出
    fields1 = extract_key_fields(record1, field_types)
    fields2 = extract_key_fields(record2, field_types)
    ...
    for field in weights:
        if field in fields1 and field in fields2 and fields1[field] and fields2[field]:
            ...
            # Jaro-Winkler類似度の計算
            similarity = jellyfish.jaro_winkler_similarity(str(fields1[field]), str(fields2[field]))
...
def get_similarity_with_llm(text1: str, text2: str, config: Config) -> float:
    """
    ...
    # APIリクエストデータ
    data = {
        "model": "gpt-4o-mini",  # または "gpt-3.5-turbo" などの適切なモデル
        "messages": [
    ...
    config.debug_log(f"LLMに類似度判定をリクエスト")
    ...
    for attempt in range(config.max_retries):
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"APIエラー ({attempt+1}/{config.max_retries}): {response.status_code} - {response.text}")
                if attempt < config.max_retries - 1:
                    time.sleep(config.retry_delay)
        except Exception as e:
            print(f"リクエストエラー ({attempt+1}/{config.max_retries}): {e}")
            if attempt < config.max_retries - 1:
                time.sleep(config.retry_delay)
...
def calculate_record_similarities(records: List[Dict], field_types: Dict[str, str],
                                config: Config, sample_size: int = 200) -> List[Dict]:
    """
    レコード間の類似度を計算する（二段階アプローチ使用）
    
    流れ:
    1. Jaro-Winkler距離で初期フィルタリング
      - 高類似度 → 直接マッチング
      - 低類似度 → 直接不一致
      - 中間 → LLMで詳細判定
    2. LLMでグレーゾーンのペアを詳細に判定
    3. 類似度が高いペアを最終結果に追加
    """
    # キャッシュディレクトリの作成
    ...
    # キャッシュファイル名
    cache_file = os.path.join(config.cache_dir, f"similarity_cache.pkl")
        ...
        for j in range(i + 1, len(sampled_records)):
            ...
            if processed % 5 == 0 or processed == total_comparisons:
                ...
                print(f"  進捗: {processed}/{total_comparisons}回 ({progress:.1f}%) - "
                      ...
                      f"Jaro-Winkler高({jw_high_matches}), 低({jw_low_matches}), LLM評価({llm_evaluated})")
            
            # まず、Jaro-Winkler距離で類似度を計算
            jw_similarity = calculate_jaro_winkler_similarity(record1, record2, field_types)
            
            # 類似度に基づいた判断
            if jw_similarity >= config.jw_upper_threshold:
                # 十分に高い類似度: 直接マッチとして処理
                similarity = jw_similarity
                jw_high_matches += 1
                config.debug_log(f"Jaro-Winkler高類似度: {similarity:.4f} - 直接マッチ")
            elif jw_similarity <= config.jw_lower_threshold:
                # 十分に低い類似度: 直接不一致として処理
                similarity = jw_similarity
                jw_low_matches += 1
                config.debug_log(f"Jaro-Winkler低類似度: {similarity:.4f} - 直接不一致")
            else:
                # グレーゾーン: LLMで詳細評価
                llm_evaluated += 1
                ...
                if cache_key in similarity_cache:
                    ...
                    cache_hits += 1
                    config.debug_log(f"キャッシュヒット: {similarity:.4f}")
                else:
                    # LLMで類似度を計算
                    similarity = get_similarity_with_llm(text1, text2, config)
                    config.debug_log(f"LLM評価: {similarity:.4f}")
            ...
            if similarity >= config.similarity_threshold:
                ...
                match_info = {
                    ...
                    "similarity_method": "jaro_winkler" if similarity == jw_similarity else "llm",
                    "title1": title1,
                    "title2": title2,
                    "author1": author1,
                    "author2": author2,
    ...
    print(f"内訳: Jaro-Winkler高類似度({jw_high_matches}), Jaro-Winkler低類似度({jw_low_matches}), LLM評価({llm_evaluated})")
...
def main():
    parser = argparse.ArgumentParser(description="Jaro-Winkler & LLMハイブリッド書誌レコードマッチング")
    parser.add_argument("--input", "-i", type=str, required=True, help="入力YAMLファイルパス")
    parser.add_argument("--output", "-o", type=str, help="出力YAMLファイルパス")
    ...
    # 入力ファイル名から出力ファイル名を自動生成（出力指定がない場合）
    if args.output is None:
        import os
        input_basename = os.path.basename(args.input)
        input_name = os.path.splitext(input_basename)[0]
        args.output = f"{input_name}_result.yaml"
    
    # 重複している二回目の parser.parse_args() を削除
    ...
    print(f"設定: しきい値={config.similarity_threshold}, JW上限={config.jw_upper_threshold}, JW下限={config.jw_lower_threshold}")
