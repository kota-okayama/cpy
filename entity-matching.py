...
async def process_yaml_file(yaml_file_path: str, 
    ...
    """
    YAMLファイルを読み込み、Human-in-the-loopプロセスを実行して結果を保存
    """
    try:
        # コマンドライン引数の処理
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='一貫性に着目したGPTベースのエンティティマッチング')
    ...
    parser.add_argument('--max-representatives', '-mr', type=int, default=10, help='クラスターあたりの最大代表ペア数（デフォルト: 10）')
    
    # Human-in-the-loop関連の引数
    parser.add_argument('--run-experiments', '-e', action='store_true', 
                     help='様々な人間精度と戦略の組み合わせで実験を実行')
