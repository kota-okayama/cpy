...
# メイン処理関数
async def process_yaml_efficiently(yaml_data: str, api_key: str, batch_size: int = 8, threshold: float = 0.7) -> Dict[str, Any]:
    ...
    try:
        ...
        # 類似度情報を出力に追加
        final_results["similarity_report"] = similarity_report
        final_results["similarity_analysis"] = similarity_analysis
        final_results["merge_decisions"] = merge_decisions
...
# コマンドライン実行用のメイン関数
async def main(yaml_file_path: str, output_file: str = None, batch_size: int = 8, threshold: float = 0.7, similarity_report_path: str = None) -> None:
    ...
    try:
        ...
        # 処理実行
        results = await process_yaml_efficiently(yaml_data, API_KEY, batch_size, threshold)
        ...
        if "summary" in results:
            ...
            print(f"  ベンチマークの正解グループ数: {metrics.get('num_of_groups(correct)', 'N/A')}")
        ...
        if "similarity_analysis" in results:
            ...
            print("\n類似度分析:")
            print(f"  合計ペア数: {analysis.get('count', 'N/A')}")
            print(f"  しきい値以上のペア数: {analysis.get('pairs_above_threshold', 'N/A')}")
            print(f"  平均類似度: {analysis.get('avg_score', 0):.4f}")
            print(f"  スコア分布:")
            if "score_distribution" in analysis:
                for score_range, count in analysis["score_distribution"].items():
                    print(f"    {score_range}: {count}ペア")
...
if __name__ == "__main__":
    ...
    # 非同期処理を実行
    asyncio.run(main(args.input, args.output, args.batch_size, args.threshold, args.similarity_report))