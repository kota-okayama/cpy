#!/usr/bin/env python3
"""
API対応エンベディングクラスタリング（キャッシュ機能付き）

このスクリプトは書誌レコードのクラスタリングを行うための包括的なソリューションです。
OpenAI APIを使用してエンベディングを計算し、結果をキャッシュに保存して再利用します。

特徴:
- OpenAI APIを確実に使用するエンベディング計算
- エンベディングのキャッシュ機能による高速化
...
try:
    ...
    print("エラー: 'requests' ライブラリがインストールされていません。以下のコマンドでインストールしてください:")
    print("pip install requests")
    sys.exit(1)

# 設定
SIMILARITY_THRESHOLD = 0.9  # デフォルトの類似度しきい値
...
def get_embedding_with_api(text: str, api_key: str) -> np.ndarray:
    """OpenAI APIを使用してエンベディングを取得する（必ずAPI使用）"""
    if not api_key:
...
def main():
    parser = argparse.ArgumentParser(description="API対応エンベディングクラスタリング（キャッシュ機能付き）")
