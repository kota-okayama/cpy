...
if __name__ == "__main__":
    ...
    # コマンドライン引数が1でない場合、使い方を表示する
    if len(sys.argv) != 2:
        output = "\n"
        output += "[[ Usage ]]\n"
        output += "1 command line arguments are required.\n"
        output += "$ python create_n4u_answer.py [path to target yaml file]\n"
        print(output)
        sys.exit(0)
    ...
    # プロジェクトディレクトリ作成
    root_pro_dir = os.path.join(DIRPATH, "project")
    if not os.path.exists(root_pro_dir):
        os.mkdir(root_pro_dir)

    # requestディレクトリから候補を探す
    req_dir = os.path.join(DIRPATH, "request")
    target_files = []
    if os.path.exists(req_dir):
        # requestディレクトリ内のファイル一覧を取得
        files = os.listdir(req_dir)
        target_files = [f for f in files if os.path.isfile(os.path.join(req_dir, f))]
    ...
    for file in target_files:
        ...
        for num in max_data:
            ...
            cs.load_req_cs(filepath, num)

            # 各プロジェクトディレクトリ作成
            pro_dir = os.path.join(root_pro_dir, f"{file[:-4]}_{num}")
            ...
            with open(os.path.join(DIRPATH, "config.yaml"), "r") as f:
                content = f.read()
            with open(os.path.join(pro_dir, "config.yaml"), "w") as f:
                f.write(content)

            # suspendをコピー
            with open(os.path.join(DIRPATH, "suspend.yaml"), "r") as f:
                content = f.read()
            with open(os.path.join(pro_dir, "suspend.yaml"), "w") as f:
                f.write(content)

            # inputディレクトリ作成
            input_dir = os.path.join(pro_dir, "input")
            if not os.path.exists(input_dir):
                os.mkdir(input_dir)

            # 出力先ファイルパス
            input_filepath = os.path.join(input_dir, f"{file[:-4]}_{num}.csv")

            # 出力処理
            cs.output_result(input_filepath)
