...
class InferenceDistributuionGraphContainter:
    ...
    def load_req_cs(self, filepath: str, max_data: int = 0):
        ...
        max_data = max_data if max_data != 0 else 10001
            ...
            for i, row in enumerate(csv):
                ...
                # dataが欠損してRecordを構成できない場合は、スキップする
                    ...
                    try:
                        ...
                        if i % 1000 == 0:
                            print(f"{i},{self.statics['true_positive'] + self.statics['false_negative']}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        output = "\n"
        output += "[[ Usage ]]\n"
        output += "2 command line arguments are required.\n"
        output += "$ python create_n4u_answer.py [path to target yaml file] [path to requied_crowdsourcing csv file] \n"
        print(output)
        sys.exit(0)
    ...
    graph.load_req_cs(req_filepath)
