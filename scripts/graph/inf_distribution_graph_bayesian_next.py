...
class InferenceDistributuionGraphContainter:
    ...
    def draw_histogram(self):
        ...
        # 色の算出
        color_codes = []
        max_range = min(len(self.req_cs), MAX_RANGE)
        max_count = max_range // STEP
        for count, i in enumerate(reversed(range(STEP - 1, max_range, STEP))):
            rgb = hsv_to_rgb(0.0766, count / max_count * 0.9 + 0.1, 1)
            color_code = f"#{hex(int(rgb[0] * 255))}{hex(int(rgb[1] * 255))}{hex(int(rgb[2] * 255))}"
            color_codes.append(color_code.replace("0x", ""))

        # color_lst = [alt.value(color_code) for color_code in color_codes]

        # ラベルの設定
        label = [f"{i + 1}" for i in reversed(range(STEP - 1, max_range, STEP))]
        ...
        chart = (
            alt.Chart(
                df,
                title=f"{GRAPH_TITLE} (n = {self.num_of_all})",
            )
            .mark_bar(
                opacity=0.5,
                color=alt.ColorName("lightgray"),
                clip=True,
            )
            .encode(
                x=alt.X(
                ...
                y=alt.Y(
                    "count(same)",
                    scale=alt.Scale(domainMin=1, type=SCALE),
        ...
        for count, i in enumerate(reversed(range(STEP - 1, max_range, STEP))):
            ...
            df = pd.DataFrame(self.req_cs[:i], columns=["same", "not_same", "inf_type"])
    ...
    def draw_step_chart(self):
        ...
        max_count = max_range // STEP
        for count, i in enumerate(reversed(range(STEP - 1, max_range, STEP))):
            rgb = hsv_to_rgb(0.0766, count / max_count * 0.5 + 0.5, 1)
            color_code = f"#{hex(int(rgb[0] * 255))}{hex(int(rgb[1] * 255))}{hex(int(rgb[2] * 255))}"
            color_code = color_code.replace("0x", "")

            # altairで描画
            df = pd.DataFrame(self.req_cs[:i] + dummy_req_cs, columns=["same", "not_same", "inf_type"])
            df = df.replace(replace_target)
            chart += (
                alt.Chart(
                    df,
