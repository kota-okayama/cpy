"""クラウドソーシングから統計画像を出力するためのスクリプト (SUN推論用)"""
...
class InferenceDistributuionGraphContainter:
    ...
    def draw_graph(self):
        ...
        label = [
            ...
            f"Unknown (match) ({self.statics['unknown(match)']})",
            f"Unknown (mismatch) ({self.statics['unknown(mismatch)']})",
        ]
        ...
        replace_target = {
            ...
            "unknown(match)": label[4],
            "unknown(mismatch)": label[5],
        }
...
if __name__ == "__main__":
    ...
    graph = InferenceDistributuionGraphContainter(rc)
